  ...
  const submitProofMutation = useMutation({
    mutationFn: async (proofData) => {
      const { data, error } = await supabase
        ...
        .insert({
          challenge_id: proofData.challenge_id,
          participant_id: proofData.participant_id,
          user_email: proofData.user_email,
          photo_url: proofData.photo_url,
          notes: proofData.notes,
          progress_increment: proofData.progress_increment,
    ...
    onSuccess: async (proofData) => {
      ...
      if (participant) {
        await supabase
          .from('challenge_participants')
          .update({
            current_progress: (participant.current_progress || 0) + 1,
